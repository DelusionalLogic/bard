...
void formatter_format(Vector* compiledStr, const struct FormatArray *const arrays[], size_t arraysCnt, char** Poutput) {
	Vector output;
	vector_init_new(&output, sizeof(char), 512);
	...
	while(node != NULL) {
		if(node->type == NT_STRING) {
			vector_putListBack_new(&output, node->str.lit, strlen(node->str.lit));
			...
			for(size_t i = 0; i < arraysCnt; i++) {
				if(strcmp(arrays[i]->name, node->arr.arr) == 0) {
					...
					if(pval != NULL) {
						vector_putListBack_new(&output, (char*)*pval, strlen((char*)*pval));
	...
	vector_putListBack_new(&output, "\0", 1);
